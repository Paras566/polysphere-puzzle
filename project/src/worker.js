...
window.self.addEventListener('message', MsgHandler);
...
{    
    ...
    window.self.postMessage({'MsgType': "finished"});
}
    ...
    for(var ListItem=0; ListItem<ShapeList.length; ListItem++){
        ...
        for(var LayoutNo=0; LayoutNo<Layouts.length; LayoutNo++){
            ...
            for(var PtNo=0; PtNo<Layout.length; PtNo++){
                let OffX = BoardX - Layout[PtNo][0];
                let OffY = BoardY - Layout[PtNo][1];
                let Fit = true;
                for(Pt=0; Pt<Layout.length; Pt++){
                    let PtX = OffX + Layout[Pt][0];
                    let PtY = OffY + Layout[Pt][1];
...
//                    Debug("Fit shape " + Shape.Name + "(" + ShapeNo + ") layout " + LayoutNo + " point " + PtNo + " at " + BoardX + ","+BoardY);
                    if(NewShapeList.length == 0){
                        // Got a solution!
                        window.self.postMessage({'MsgType': "solution", 'Board': JSON.stringify(NewBoard)});
                    }
                    else{
                        // Recurse
                        if(ShowWork){
                            ...
                            if((TimeNow - LastUpdateTime) > (1000 / FPSTarget)){
                                window.self.postMessage({'MsgType': "workupdate", 'Board': JSON.stringify(NewBoard)});
...
function Debug(Msg)
{
    window.self.postMessage({'MsgType': "debug", 'Msg': Msg});
}